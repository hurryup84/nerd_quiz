...
export class SettingsService implements OnModuleInit {
  ...
  async getAll(): Promise<{ theme: string; refreshInterval: number }> {
    ...
    return {
      ...
      refreshInterval: Number(map['refreshInterval'] ?? DEFAULTS['refreshInterval']),
    };
  }

  // Legacy helpers kept for compatibility
  async getTheme() { return this.get('theme'); }
  async setTheme(value: string) { return this.set('theme', value); }
}
